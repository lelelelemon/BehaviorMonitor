...
public class BingSearchEngine extends SearchEngine {
		...
		try {
			...
			url = new URL(
					"https://api.datamarket.azure.com/Bing/Search/v1/Web?Query=%27"
							+ queryWord + "%27&$format=json");
			String auth = Config.BING_ACCOUNT_KEY + ":"
					+ Config.BING_ACCOUNT_KEY;
			...
			Logger.getLogger(BingSearchEngine.class.getName()).log(
	...
	protected ArrayList<Snippet> parseResult(JSONObject resultJSON) throws JSONException {

		JSONArray resultArray = resultJSON.getJSONObject("d").getJSONArray(
				"results");
		ArrayList<Snippet> snipArray = new ArrayList<Snippet>();
		for (int i = 0; i < resultArray.length(); i++) {
			String description = resultArray.getJSONObject(i)
					.get("Description").toString();
			String url = resultArray.getJSONObject(i).get("Url").toString();
			String[] tempSplitted = description.toLowerCase()
					.replaceAll("[^A-Za-z ]", "").split(" ");
			ArrayList<String> tempOrigWords = new ArrayList<String>(
					Arrays.asList(tempSplitted));
